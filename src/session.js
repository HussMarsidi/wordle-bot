const { calculateLetterProbabilities, findBestWord } = require("./words.js");
  ...
  giveFeedback = (word, statuses) => {
    for (let i = 0; i < 5; i++) {
      ...
      if (s == feedback.CORRECT_SPOT) {
        // for the correct spot make sure that this position has only one option
        this.candidates[i] = [ch];
      } else if (s == feedback.DIFFERENT_SPOT) {
        // don't use this letter for the current position anymore
