...
const calculateLetterProbabilities = (list) => {
  ...
  // Some letters should be more popular than others
  ...
  for (let i = 0; i < w.length; i++) {
    if (candidates[i].indexOf(w[i]) > 0) {
      if (candidates[i].length == 0) {
        // when it's the only option for this position - make sure we use it
        s += 1000000;
      } else {
        // give more priority to letters that are somewhere in the word
        s += 100000;
      }
    }
    if (candidates[i].indexOf(w[i]) < 0) {
...
module.exports = { readWords, calculateLetterProbabilities, findBestWord };
